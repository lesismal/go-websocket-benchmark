...
| `-sort` | Report row order: `result` (default) ranks the best result first - TPS for Connections and BenchEcho, Bytes Recv for BenchRate - and `framework` keeps the `config.FrameworkList` order. Ties keep the framework order in both |
